    ...
    <section id="pricing" className="py-24">
      <div className="max-w-7xl mx-auto px-6 md:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {plans.map((p, i) => {
            const content = (
              <Card className={`rounded-2xl ${p.recommended ? 'relative' : ''}`}>
            ...
            // Only wrap the middle (Pro) plan with ElectricBorder
            return (
              <div key={p.name} className="relative">
                {p.recommended ? (
                  <ElectricBorder color="#7df9ff" speed={1} chaos={0.35} thickness={2} style={{ borderRadius: 16 }}>
                    {content}
                  </ElectricBorder>
                ) : (
                  content
