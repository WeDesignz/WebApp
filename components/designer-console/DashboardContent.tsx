...
export default function DashboardContent() {
  ...
  ], [kpis, pendingReviews, performanceScore, performancePercentage]);
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {recentDesigns.map((design: any) => (
                        <tr key={design.id} className="border-b border-border last:border-0 hover:bg-muted/50">
                          <td className="py-3 pr-4">
                            <Link href={`/designer-console/designs/${design.id}`} className="flex items-center gap-3">
                              <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-primary/20 to-purple-500/20 flex-shrink-0 flex items-center justify-center">
                                {design.thumbnail ? (
                                  <img src={design.thumbnail} alt={design.title} className="w-full h-full rounded-lg object-cover" />
                                ) : (
                                  <Upload className="w-6 h-6 text-muted-foreground" />
                                )}
                              </div>
                              <span className="font-medium text-sm hover:underline">{design.title}</span>
                            </Link>
                          </td>
                          <td className="py-3 pr-4">
                            <Badge 
                          ...
                          <td className="py-3 pr-4 text-sm">{formatNumber(design.views || 0)}</td>
                          <td className="py-3 pr-4 text-sm">{formatNumber(design.downloads || 0)}</td>
